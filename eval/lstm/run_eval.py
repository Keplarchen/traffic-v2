"""Evaluate ResidualLSTM (point-prediction + conformal calibration) vs baselines."""
...
CKPT_PATH = ROOT / "checkpoints" / "lstm.pt"
...
def main():
    ...
    if not CKPT_PATH.exists():
        sys.exit(f"Missing {CKPT_PATH}; please run train_lstm.py first")
    ckpt = torch.load(CKPT_PATH, map_location=device, weights_only=False)
    cfg = ckpt["config"]
    model = ResidualLSTM(**cfg).to(device)
    ...
    pred_val = inverse_transform(pred_val_z, mean, std).numpy()
    actual_val = inverse_transform(target_val_z, mean, std).numpy()
    pred = inverse_transform(pred_z, mean, std).numpy()
    actual = inverse_transform(target_z, mean, std).numpy()
    test_t = test_ds.t_starts.numpy()

    # Conformal calibration: turns point predictions into bandwidth allocations
    # by adding the per-pair val-set residual P95.
    q_hat = fit_correction(pred_val, actual_val, alpha=0.05)
    pred_conformal = apply_correction(pred, q_hat)
